...
import com.banhuitong.activity.MainActivity;
...
public class BadgeUtil {
	...
	public static void uninstallShortcut(Activity ac) {
		Intent shortcut = new Intent(
				"com.android.launcher.action.UNINSTALL_SHORTCUT");

		shortcut.putExtra(Intent.EXTRA_SHORTCUT_NAME,
				ac.getString(R.string.app_name));
		shortcut.putExtra("duplicate", false);
		Intent intent = new Intent(Intent.ACTION_MAIN);
		intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);

		intent.setClass(ac.getApplicationContext(), ac.getClass());
	...
	public static void installShortcut(Activity ac) {
		...
		shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
				Intent.ShortcutIconResource.fromContext(ac, R.drawable.u40));
		ac.sendBroadcast(shortcutIntent);
	}
}
