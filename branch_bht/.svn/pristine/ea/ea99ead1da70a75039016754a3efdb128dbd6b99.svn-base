			 ...
			 @JavascriptInterface
		        public void init(int f){  				    
		            handler.post(new Runnable() {  
		                public void run() {  
		                	CaDetailActivity.this.setResult(Constants.RESULT_INVEST_SUCCESS, lastIntent);
...
//		                		ViewUtils.showDialog("", "", "提示", getResources().getString(R.string.msg_balance_not_enough), CaDetailActivity.this, ViewUtils.Button_type_sure, null);
		                		CaDetailActivity.this.setResult(Constants.RESULT_MONEY_SHORT, lastIntent);
		                		CaDetailActivity.this.finish();
		                	}else{
		                		ViewUtils.showDialog("", "", "提示", msg, CaDetailActivity.this, ViewUtils.Button_type_sure, null);
		                	}
		...
		wvEntDetails.setWebViewClient(new WebViewClient() {
			...
			public boolean shouldOverrideUrlLoading(WebView view, String url) {       
				if(url.contains("login.html")){
					CaDetailActivity.this.setResult(Constants.RESULT_TO_LOGIN, lastIntent);
					CaDetailActivity.this.finish();
				}else if(url.contains("back.html")){
					CaDetailActivity.this.finish();
				}else if(url.contains("credit-assign-list.html")){
					if(url.indexOf("?")==-1){
						CaDetailActivity.this.setResult(Constants.RESULT_INVEST_SUCCESS, lastIntent);
						CaDetailActivity.this.finish();
					}else{
					...
					CaDetailActivity.this.setResult(Constants.RESULT_TO_JX, lastIntent);
					CaDetailActivity.this.finish();
...
//				handler.cancel(); //默认的处理方式，WebView变成空白页
	...
	private void toInvest(){
		...
		String url = Constants.mobileUrl + "creditassign/credit-assign-view.html?id=" + pId + "&hash=" + hash + "&random=" + x;
