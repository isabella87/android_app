...
public class ShareCore {
	...
	public boolean share(Platform plat, HashMap<String, Object> data) {
		...
		/*if("WechatMoments".equals(data.get("platform"))){
			data.put("title", "萌承科技 | 维修大使");
		}else if("Wechat".equals(data.get("platform"))){
			data.put("title", "萌承科技");
			data.put("text", "萌承科技 | 维修大使");
		}*/
		
		if("WechatMoments".equals(data.get("platform"))){
			data.put("title", "班汇通 | 建筑业金融信息服务专家\n符合银监会规定的互联网金融平台");
		}else if("Wechat".equals(data.get("platform"))){
			data.put("title", "班汇通");
			data.put("text", "班汇通 | 建筑业金融信息服务专家\n符合银监会规定的互联网金融平台");
		}
